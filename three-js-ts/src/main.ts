...
async  function game() {
  ...
  function animate() {
    ...
    if ( meshes.length > 0 && lastMesh?.go === false ) {
      ...
      if (lastMesh.mesh && lastMesh.mesh.position.x < -lastMesh.width+offset && notes[noteIdx]) {
        ...
        const _mesh = addLine( new THREE.Vector3( offset + _xx/2  ,heightLine ,0) ,  new THREE.Vector3( _lineWidth ,0 ,0) );
